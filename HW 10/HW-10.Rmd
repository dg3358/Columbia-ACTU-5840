...
\item Splitting on $\text{Gender} < 0.5$: Encode $\{M: 0, F: 1\}$. Thus, all males (points 1,4,5, 7) are in the left child, and all females (points 2,3, 6) are in the right child. Thus, $\bar{y}_l = \frac{13.712 - 1.288 + 3.127 + 5.712}{4} = 5.316$, and $SSE_l = \sum_{i \in L}(y_i - \bar{y}_l)^2 = 119.05$. Similarly, $\bar{y}_r = \frac{7.712 - 11.963 - 16.963}{3} = -7.071$ and $SSE_r = \sum_{i \in R}(y_i - \bar{y}_r)^2 = 340.32$. Thus, $SSE = 119.05 + 340.32 = 459.37$.
\end{itemize}
...
Split & Points in L & Points in R & $SSE_l$ & $SSE_r$ & $SSE$\\
\hline\hline
$\text{Height} \leq 1.4$ & 6 & 1,2,3,4,5,7 & $0$ & $386.44$ & $386.44$\\
\hline
$\text{Height} \leq 1.5$ & 3,5,6 & 1,2,4,7 & $218.77$ & $114.75$ & $333.52$\\
\hline
$\text{Height} \leq 1.6$ & 1,3,5,6 & 2,4,7 & $592.13$ & $44.67$ & $636.80$\\
\hline
$\text{Height} \leq 1.7$ & 1,2,3,5,6,7 & 4 & $720.46$ & $0$ & $720.46$\\
\hline
$\text{Gender} < 0.5$ & 1,4,5,7 & 2,3,6 & $119.05$ & $340.32$ & $459.37$
\end{tabular}
\end{center}
Note that the lowest remaining SSE after split belongs to the split corresponding with $\mathbf{\text{Height} \leq 1.5}$. So, leaf $L$ contains $3,5,6$ and leaf $R$ contains $1,2,4,7$ after the first split. Note that $SSE_L = 218.77 > 114.75 = SSE_R$. Therefore, if I were to find a split among the three data points in leaf $L$ whose decrease in SSE was greater than $SSE_R$, I know that this is the optimal second split. Data points $3,5,6$ have heights $\{1.4,1.5\}$ and genders $\{M,F\}$. Therefore, there is one potential split on height and one potential split on gender; I examine both (calling the resulting leaves $L_1$ and $L_2$):
\begin{itemize}
\item Splitting on $\text{Height} \leq 1.4$: Only observation 6 fits this criteria; therefore, $\bar{y}_{L_1} = y_6 = -16.963$ and $SSE_{L_1} = 0$. Then, observations 3 and 5 are in $L_2$, so $\bar{y}_{L_2} = \frac{-11.963	+ 3.127}{2} = -4.418$. Thus, $SSE_{L_2} = \sum_{i \in L_2}(y_i - \bar{y}_{L_2})^2 = 113.85$. So, $SSE_{\text{tot}} = 0 + 113.85 = 113.85$.
\item Splitting on $\text{Gender} < 0.5$: Only observation 5 is male of the 3 observations in $L$; therefore, $\bar{y}_{L_1} = y_5 = 3.127$ and $SSE_{L_1} = 0$. Then, observations 3 and 6 are female and in $L_2$. Thus, $\bar{y}_{L_2} = \frac{-11.963 - 16.963}{2} = -14.463$ and $SSE_{L_2} = \sum_{i \in L_2}(y_i - \bar{y}_{L_2})^2 = 12.5$. Thus $SSE_{\text{tot}} = 0 + 12.5 = 12.5$.
\end{itemize}
Thus, splitting on $\text{Gender} < 0.5$ is the best split among data in Leaf $L$ and $218.77 - 12.5 = 206.27 > 114.75$ so the reduction in $SSE$ is greater than the entirety of the $SSE$ in leaf $R$; thus, the split on leaf $L$ on $\mathbf{\text{Gender} < 0.5}$ is the optimal second split. After this split, there are 3 leaves, so the third tree is completed - note that the leaves are grouped exactly the same as they were in the second tree. Therefore, I summarize the leaves, the observations in each leaf, and the updated weight prediction for the leaves ($\hat{y}_{\text{Tree}_3} = \hat{y}_{\text{Tree}_2} + 0.1*\text{pred}(\text{Resid}_2)$):
\begin{center}
\begin{tabular}{c|c|c|c}
Leaf Name & Data points in leaf & Prediction for $\text{Resid}_2$ & Updated Weight Prediction\\
\hline\hline
...
\begin{itemize}
\item $SSE_{L_1} = \sum_{i \in L_1}e_{\text{tree 3},i}^2= 2.8143^2 = 7.92$
\item $SSE_{L_2} = \sum_{i \in L_2}e_{\text{tree 3},i}^2 = (-10.5167)^2 + (-15.5167)^2 = 351.37$
\item $SSE_{R} = \sum_{i \in R}e_{\text{tree 3},i}^2 = 13.0658^2 + 7.0658^2 + (-1.9342)^2 + 5.0658^2 = 250.04$
\end{itemize}
...
In this case, I would predict that $\hat{y}(\text{M, 1.45}) = 74.1857$. This is exactly what I found in part d, because both trees 2 and 3 had the exact same splits, which means tree 3 and the total, gradient boosted tree are exactly the same.