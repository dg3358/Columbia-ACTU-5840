...
The deviance is very close between the tree with 6 leaves and 7 leaves. I calculate the size of the tree with minimum deviance:
```{r, include = TRUE}
minD <- cv$size[which.min(cv$dev)]
sprintf("Size of tree with minimum deviance: %.0f", minD)
```

### iv. Pruning the tree
Since the tree with the lowest deviance is the the full tree, no pruning is necessary.
I re-plot the same tree:
```{r, include = TRUE}
replot <- rpart(medv ~ ., data = trainData, method = "anova")
prp(replot, main = "Pruned Regression Tree for Median Home Value", roundint = FALSE,
    extra = 1, digits = 4, branch.lty = 3)
```

### v. Test error
The test error is the same as before, as the tree is the same as before: $\text{MSE} = 35.28688$.

## c. Building Bagging Model
I create the bagged model, and calculate the test MSE:
```{r, include = TRUE}
set.seed(1)
...
sprintf("Test MSE of Bagged Model: %.5f", MSErF)
```

I also return the top three variables in importance as measured by the mean decrease in MSE:
```{r, include = TRUE}
sortedImportance <- sort(rF$importance[,2], decreasing = TRUE)
sortedImportance[1:3]
```
So, this suggests that $\textit{rm}, \textit{lstat}, \text{ and } \textit{ptratio}$ are the 3 most important variables in decreasing MSE.
